...
public class GlobalExceptionHandler {
  ...
  public ResponseEntity<BaseResponse<Void>> handleBankUnavailable(BankUnavailableException ex) {
    ...
    return ResponseEntity.status(ErrorCode.BANK_UNAVAILABLE.getHttpStatus())
  ...
  public ResponseEntity<BaseResponse<Void>> handleAuthentication(AuthenticationException ex) {
    ...
    return ResponseEntity.status(ErrorCode.AUTHORIZATION_REQUIRED.getHttpStatus())
        .body(
            BaseResponse.failure(
                ErrorCode.AUTHORIZATION_REQUIRED.name(), "Authentication required"));
  }
  ...
  private <T> ResponseEntity<BaseResponse<T>> validationErrorResponse(T errors, String message) {
    return ResponseEntity.status(ErrorCode.VALIDATION_FAILED.getHttpStatus())
        .body(
            new BaseResponse<>(
                false, errors, new ApiError(ErrorCode.VALIDATION_FAILED.name(), message)));
  }
  ...
  public ResponseEntity<BaseResponse<Void>> handleNoResourceFoundException(
    ...
    return ResponseEntity.status(HttpStatus.NOT_FOUND)
        .body(BaseResponse.failure("NO_RESOURCE_FOUND", ex.getMessage()));
  }
  ...
  public ResponseEntity<BaseResponse<Void>> handleGenericException(Exception ex) {
    ...
    return ResponseEntity.status(HttpStatus.INTERNAL_SERVER_ERROR)
        .body(BaseResponse.failure("INTERNAL_SERVER_ERROR", "An unexpected error occurred"));
  }
}
