...
public class IdempotencyKeyServiceImpl implements IdempotencyKeyService {
    ...
    @Override
    public IdempotencyKey getOrCreateIdempotencyKey(UUID merchantId, String idempotencyKey, String requestParams, Operation operation) {

        return idempotencyKeyRepository.findByMerchantIdAndIdempotencyKeyAndOperation(merchantId, idempotencyKey, operation)
                .orElseGet(() -> {

                    Merchant merchant = merchantRepository.findById(merchantId)
                            .orElseThrow(() -> new ResourceNotFoundException("Merchant not found"));

                    IdempotencyKey key = new IdempotencyKey(merchant, idempotencyKey, requestParams, Constants.STARTED, OffsetDateTime.now(), operation);
    ...
    @Override
    public IdempotencyKey getOrCreateIdempotencyKey(UUID merchantId, String idempotencyKey, UUID paymentId, String requestParams, Operation operation) {
    ...
    @Override
    public Optional<IdempotencyKey> findIdempotencyForUpdate(String idempotencyKey, UUID merchantId, Operation operation) {
