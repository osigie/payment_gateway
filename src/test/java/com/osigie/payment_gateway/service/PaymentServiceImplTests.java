...
class PaymentServiceImplTests {
  ...
  void createAuthorize_withCachedResponse_shouldReturnCachedResult() {
    var cachedKey =
        TestDataFactory.createIdempotencyKey(merchant, payment, AuthorizeRecoveryPoints.FINISHED);
    cachedKey.setResponseBody(
        objectMapper.writeValueAsString(Result.success(paymentMapper.toDto(payment))));
  ...
  void createAuthorize_withBankError_shouldReturnFailure() {
    ...
    when(bankClient.authorize(any(), any()))
        .thenThrow(
            new BankBusinessException(
                "Insufficient funds", "insufficient_funds", HttpStatus.PAYMENT_REQUIRED));
  ...
  void createCapture_withValidRequest_shouldProgressThroughAllPhases() {
    ...
    var captureKey =
        TestDataFactory.createIdempotencyKey(merchant, payment, CaptureRecoveryPoints.STARTED);
    ...
    var authTransaction =
        TestDataFactory.createTransaction(payment, TransactionType.AUTHORIZED, "auth-ref-123");
  ...
  void createVoid_withValidRequest_shouldProgressThroughAllPhases() {
    ...
    var voidKey =
        TestDataFactory.createIdempotencyKey(merchant, payment, VoidRecoveryPoints.STARTED);
  ...
  void createRefund_withValidRequest_shouldProgressThroughAllPhases() {
    ...
    var refundKey =
        TestDataFactory.createIdempotencyKey(merchant, payment, RefundRecoveryPoints.STARTED);
    ...
    var captureTransaction =
        TestDataFactory.createTransaction(payment, TransactionType.CAPTURE, "capture-ref-456");
  ...
  void getPayment_withNonExistingPayment_shouldReturnFailure() {
    when(paymentRepository.findPaymentByIdAndMerchantId(any(), any())).thenReturn(Optional.empty());
